	<section>
		<div class="container">
			<div class="row">
				...
				$yrmSys = 0;
				$sinir = 2;
				$kontrol3 = "SELECT * FROM yorumlar WHERE hid='$hid' ORDER BY id DESC LIMIT $sinir";
				$kontrol2 = mysqli_query($baglanti, $kontrol3);
				while($kontrol = mysqli_fetch_array($kontrol2)){
					if($kontrol["kid"]==$uye["id"]){
						$yrmSys++;
					}
				}
				if($yrmSys==$sinir and $uye["yetki"]==0){
					echo "Bu habere ait son $sinir yorum sizin olduğu için yeni bir yorumu şu an yapamazsınız";
					exit();
				}
